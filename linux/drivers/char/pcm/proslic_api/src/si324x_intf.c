...
**
** $Id: si324x_intf.c,v 1.1.1.1 2014/01/09 02:43:49 mt7620 Exp $
**
** Si324x_Intf.c
...
int Si324x_PrintDebugData (proslicChanType *pProslic){
		...
		for (i=0;i<1024;i++)
			LOGPRINT ("%sRAM %d = %X\n",LOGPRINT_PREFIX,i,ReadRAM(pProHW,pProslic->channel,i));
#endif
		return 0;
}
...
int Si324x_LoadPreviousLBCalPacked (proslicChanType *pProslic,int32 *result){
    int32 ramVal;
#ifdef API_TEST
    const char fmt_string = "%sUNPACKED %s = %08x\n";
#endif
	setUserMode(pProslic,TRUE);
    ...
    LOGPRINT (fmt_string, "CAL_TRNRD_DACT",ramVal);
    ramVal = ReadRAM(pProHW,pProslic->channel,CAL_TRNRD_DACR);
    LOGPRINT (fmt_string, "CAL_TRNRD_DACR",ramVal);
#endif
	setUserMode(pProslic,FALSE);
	return 0;
}
...
/*
** Function: Si324x_DisableInterrupts
**
** Description: 
** Disables interrupts
*/
int Si324x_DisableInterrupts (proslicChanType_ptr pProslic){
