...
#define SYSPARMS_MINUS_OFFSET 0x400
#define BOOTLOADER_SIZE       0x20000
#define SYSPARAMS_BASE (CONFIG_MTD_PHYSMAP_START+BOOTLOADER_SIZE-SYSPARMS_MINUS_OFFSET)

typedef struct sys_params_s {
      char alias[32];  /* should be same as ih_name */
      char lanmac[32];
      char wanmac[32];
      int diag;
      int ated;
...
int get_mac_from_sysparams(unsigned char *mac)
{
        int i;
        sys_params_t params;

      i = ra_mtd_read_nm("Bootloader", BOOTLOADER_SIZE-SYSPARMS_MINUS_OFFSET, sizeof(sys_params_t), &params);
      memcpy(mac,params.lanmac,6);
      printk("Base MAC: %02x:%02x:%02x:%02x:%02x:%02x",
                      mac[0],
                      mac[1],
                      mac[2],
                      mac[3],
...
#ifdef CONFIG_EFM_PATCH
        i = get_mac_from_sysparams(addr.sa_data);
#else
