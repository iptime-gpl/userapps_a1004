		  ...
		  enum ip_conntrack_info *ctinfo)
{
	...
	ct = nf_ct_tuplehash_to_ctrack(h);
        ...
        ct->tuplehash[NF_CT_DIRECTION(h)].bytes += ntohs(iphdr->tot_len);
        }
#endif
...
#ifdef CONFIG_EFM_PATCH
	if (http_bypass_hwnat) {
		struct iphdr *iph = ip_hdr(skb);
		if (iph->protocol == IPPROTO_TCP)
		{
			struct tcphdr *tcph = tcp_hdr(skb);
			if (ntohs(tcph->dest) == 80 ||  ntohl(tcph->dest) == 8080)
				FOE_ALG(skb)=1;
		}
	}
#endif
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_CONTROL
static int proc_read_conntrack_info(char *page, char **start, off_t off, int count, int *eof, void *data_unused)
{
        sprintf(page, "total:%d(%d), tcp:%d(%d), udp:%d(%d), icmp:%d(%d)",
                        init_net.ct.count.counter,nf_conntrack_max,
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
static int proc_read_conntrack_ip_stat(char *page, char **start, off_t off, int count, int *eof, void *data_unused)
{
        char *p = page;
        int i, total;
...
int check_local_ip(u32 ipaddr)
{
        ...
        dev = dev_get_by_name(&init_net, CONFIG_LOCAL_IFNAME);
        if(dev == NULL)
        {
...
int nf_conntrack_init(struct net *net)
{
	...
	if (net_eq(net, &init_net)) {
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_CONTROL
        {
                ...
                if(proc)
                        proc->read_proc = (read_proc_t*)&proc_read_conntrack_info;
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
        {
                ...
                if(proc)
                        proc->read_proc = (read_proc_t*)&proc_read_conntrack_ip_stat;
