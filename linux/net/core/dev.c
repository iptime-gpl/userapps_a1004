...
#endif
	if (q->enqueue
//#ifdef CONFIG_QOS_INTERFACE_MARK
//            && (skb->mark || !strncmp(dev->name, "ra", 2)) // mssid interface
//#endif
	) {
...
#ifdef CONFIG_QOS_INTERFACE_MARK
#define INTERFACE_MARK_SUBID_BASE       30
/* for 5G -> BASE is 34 */
#endif
...
#ifdef CONFIG_QOS_INTERFACE_MARK
        if (rcu_dereference(skb->dev->qdisc) && !strncmp(skb->dev->name, "ra", 2))
        {
                int index = 0, base=0;

                if(!strncmp(skb->dev->name,"rai",3))
                {
                        sscanf(skb->dev->name,"rai%d", &index);
                        base = INTERFACE_MARK_SUBID_BASE*2; /* 5G, */
                }
                else
                {
                        sscanf(skb->dev->name,"ra%d", &index);
                        base = INTERFACE_MARK_SUBID_BASE;
                }
                if(index) skb->mark = base+index;
                //printk("ifname: %s index : %d, mark : %d\n", skb->dev->name, index, skb->mark);
        }
#endif
...
#if defined(CONFIG_MULTICAST_FORWARD) || defined(CONFIG_MULTICAST_FORWARD_MODULE)
        if (multicast_forward_handler && multicast_forward_handler(skb, skb->dev))
        {
                ret = NET_RX_SUCCESS;
                goto out;
        }
...
#endif
#if defined(CONFIG_SMART_QOS_MONITOR) || defined(CONFIG_SMART_QOS_MONITOR_MODULE)
EXPORT_SYMBOL(smart_qos_mon_callback);
#endif
