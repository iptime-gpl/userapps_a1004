...
/*FP to-lan on/off*/	
	...
	if(pptp_fast_path && (skb->len != (124 - 14))){
		...
		if(ret == 0)
			return ret;
	   //printk("fast_pptp_to_lan return %d\r", ret);
	}
...
#endif
#if defined (CONFIG_RA_HW_NAT_PPTP_L2TP) 
	    /*PPTP use this pingpong OK , L2TP OK too*/
	    //printk("ping pong skb_length is %d\n", skb->len);
...
/*WAN->LAN accelerate*/
	...
	if (IS_MAGIC_TAG_VALID(skb) && (FOE_AI(skb) == HIT_UNBIND_RATE_REACH) && (FOE_ALG(skb) == 0) ) {
	    ...
	    iph = (struct iphdr *)(skb->data + 18); 

	    uh = (struct udphdr *)(skb->data + 18 + 20); 
	...
	if((iph->protocol == IPPROTO_GRE) || (ntohs(uh->dest)==1701))
	{
		...
		if(l2tp_fast_path){
			...
			printk("LAN->WAN  TxH L2tpToWanParseLayerInfo\n");
...
#if defined (CONFIG_RAETH_QDMA) && defined  (CONFIG_PPE_MCAST)
	if(PpeParseResult.is_mcast) {
		foe_mcast_entry_ins(PpeParseResult.vlan1, PpeParseResult.dmac, 0, 0, M2Q_table[skb->mark]);
	}
#endif
...
void PpeSetFoeEbl(uint32_t FoeEbl)
{
    ...
    if (FoeEbl == 1) {
	        /*cannot set NAPT only to support PPTP FP, WAN -> LAN use NAT */
