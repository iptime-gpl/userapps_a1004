...
static int ct_seq_show(struct seq_file *s, void *v)
{
	...
	if (seq_printf(s, "secmark=%u ", ct->secmark))
        ...
        if (seq_printf(s, " org=%lu:%llu rpl=%lu:%llu \n",
                        ct->tuplehash[IP_CT_DIR_ORIGINAL].packets,
                        ct->tuplehash[IP_CT_DIR_ORIGINAL].bytes,
                        ct->tuplehash[IP_CT_DIR_REPLY].packets,
                        ct->tuplehash[IP_CT_DIR_REPLY].bytes))
                goto release;
#else
